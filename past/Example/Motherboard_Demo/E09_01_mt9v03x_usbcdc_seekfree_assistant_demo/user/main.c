...
void main (void)
{
    ...
    while(1)
    {
        if(mt9v03x_finish_flag)
        {
            ...
            // 全速 USB_CDC 的传输速率约 600KB/S，而 188×120 分辨率的灰度图像单帧数据量约 22KB，
            // 若按 100FPS 的采集速率计算，实际数据传输需求达 2200KB/S，远超 USB_CDC 的带宽上限。
            // 若连续采集不做等待，易出现图像数据拷贝过程中，采集端正在更新图像数组的情况，
            // 最终导致图像错位、数据异常，因此需待单帧图像采集完成后，再启动下一帧的采集流程。
