...
void UART4_Isr() interrupt 18
{
	...
	{
        UART4_CLEAR_RX_FLAG;
		//锟斤拷锟斤拷锟斤拷锟捷寄达拷锟斤拷为锟斤拷S4BUF;
		if(wireless_type == WIRELESS_SI24R1)
        {
            wireless_uart_callback();           //锟斤拷锟斤拷转锟斤拷锟节回碉拷锟斤拷锟斤拷
        }
        else if(wireless_type == WIRELESS_CH9141)
        {
            bluetooth_ch9141_uart_callback();   //锟斤拷锟斤拷转锟斤拷锟节回碉拷锟斤拷锟斤拷
        }
...
{
	INT2_CLEAR_FLAG;  //锟斤拷锟斤拷卸媳锟街?
}
void INT3_Isr() interrupt 11
{
	INT3_CLEAR_FLAG;  //锟斤拷锟斤拷卸媳锟街?
}

void INT4_Isr() interrupt 16
{
	INT4_CLEAR_FLAG;  //锟斤拷锟斤拷卸媳锟街?
}
