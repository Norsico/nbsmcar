...
#define AT24C02_SCL_LOW()           gpio_set_level(AT24C02_SCL_PIN,0)     //IO口输出低电平
#define AT24C02_SCL_HIGH()          gpio_set_level(AT24C02_SCL_PIN,1)     //IO口输出高电平
#define AT24C02_SDA_LOW()           gpio_set_level(AT24C02_SDA_PIN,0)     //IO口输出低电平
#define AT24C02_SDA_HIGH()          gpio_set_level(AT24C02_SDA_PIN,1)     //IO口输出高电平
...
void flash_init(){
    gpio_init(P33_8,GPO,0,GPO_OPEN_DTAIN);
    gpio_init(P33_6,GPO,0,GPO_OPEN_DTAIN);
}
...
static int at24c02_sccb_waitack(void)
{
    ...
    if(gpio_get_level(GET_AT24C02_SDA))           //应答为高电平，异常，通信失败
    {
...
static uint8 at24c02_read_ch(uint8 ack_x)
{
    ...
    for(i=0;i<8;i++)
    {
        ...
        at24c02_simiic_delay();
        c<<=1;
        if(gpio_get_level(GET_AT24C02_SDA))
        {
            c+=1;   //读数据位，将接收的数据存c
        }
    }
