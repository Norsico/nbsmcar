...
//    IMU_get_euler();
      ...
      //下面注释的代码为直立代码和跳跃代码,目前处于调试摄像头阶段,只有这个地方被注释了
      if(Run)
      {
          //Target_Speed=SPEED*(1.0-1.0*Turn_out/4000);
         GetGoalPulse();//速度决策
         Target_Speed=speed_goal;
      }
      else
          Target_Speed=0;
      Control();
      control_s();  //舵机控制,两毫秒中断
      if(Record_Dis){
          Distance = Distance+ (motor_value.receive_left_speed_data-motor_value.receive_right_speed_data)*0.005;
      }
      if(Record_Angle){
          SumAngle=SumAngle+ imu660ra_gyro_transition(imu660ra_gyro_z)*0.001;
          //SumAngle=SumAngle + kalmanFilter_A(mpu_gyro_z) * 0.001;
      }
      cnt1++;
      jump();
...
/*
    ...
    if(cnt1>4)
    {
        ...
        IMU_get_euler();
          speed_PID_rate(-0,-0);
        speed_l=output_l;//+up_PID();//output_l+
        speed_r=output_r;//+up_PID();
        if(speed_l>3000)
            speed_l=3000;
        if(speed_l<-3000)
            speed_l=-3000;
