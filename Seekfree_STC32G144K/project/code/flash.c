...
static uint8 flash_camera_page_is_valid(const flash_camera_page_t *page)
{
    ...
    if(!flash_camera_value_is_valid(FLASH_CAMERA_EXP_TIME, page->exp_time))
    {
        return 0;
    }

    if(!flash_camera_value_is_valid(FLASH_CAMERA_GAIN, page->gain))
    {
        return 0;
    }
...
{
    plan->camera_page.exp_time = 100;
    plan->camera_page.gain = 32;
...
int16 flash_get_camera_value(flash_camera_slot_t slot)
{
    ...
    {
        case FLASH_CAMERA_EXP_TIME:
            return flash_get_current_plan()->camera_page.exp_time;
        case FLASH_CAMERA_GAIN:
            return flash_get_current_plan()->camera_page.gain;
        default:
            return 0;
    }
}
