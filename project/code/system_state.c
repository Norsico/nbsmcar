...
uint8 system_take_encoder_pending_count(void)
{
    uint8 encoder_pending_count = 0;
    uint8 interrupt_flag = 0;

    /* 关中断后一次性取走累计周期，避免主循环和 Tick 中断同时读写时丢计数。 */
    interrupt_flag = EA;
    EA = 0;
    encoder_pending_count = g_flag_encoder;
    g_flag_encoder = 0;
    EA = interrupt_flag;

    return encoder_pending_count;
}
