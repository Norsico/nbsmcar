...
static uint16 flash_store_calc_checksum_raw(const uint8 *raw_ptr, uint16 size)
{
    uint16 checksum = 0;
    uint16 i = 0;
...
static uint16 flash_store_calc_checksum(const flash_store_image_t *image)
{
    return flash_store_calc_checksum_raw((const uint8 *)image,
                                         (uint16)(sizeof(flash_store_image_t) - sizeof(image->checksum)));
}

static uint16 flash_store_calc_checksum_v6(const flash_store_image_v6_t *image)
{
    return flash_store_calc_checksum_raw((const uint8 *)image,
                                          ...
                                          flash_store_image_t *new_image)
{
    ...
    if(flash_store_calc_checksum_v6(old_image) != old_image->checksum)
    {
        return 0;
    }
