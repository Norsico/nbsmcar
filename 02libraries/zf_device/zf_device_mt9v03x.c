...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     按配置表整体下发摄像头参数
// 参数说明     config          配置表，命令格式与初始化配置一致
// 返回参数     uint8           1-失败 0-成功
// 使用示例     mt9v03x_set_config(config);                    // 调用前请先完成摄像头初始化
// 备注信息     适合在运行时重发自动曝光、曝光时间、增益等整组配置
//-------------------------------------------------------------------------------------------------------------------
uint8 mt9v03x_set_config (const int16 config[MT9V03X_CONFIG_FINISH][2])
{
    uint8 return_state = mt9v03x_sccb_set_config(config);
    return return_state;
}
