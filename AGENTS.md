...
- 当前 `MT9V03X` 图像最右侧最后一列为无效黑边；不要改底层分辨率定义，业务层统一按前 `187` 列作为有效图像宽度处理和显示。
- 逐飞助手 WiFi 图传发送的是摄像头图像缓冲和边界/参数数据，不是 `IPS200` 屏幕像素内容；停用屏幕整图显示时，优先保留参数文字，把图像改走 WiFi。
- 高速 `WIFI SPI` 图传与 `IPS200` 屏幕不能同时作为主要显示链路使用，官方示例明确说明二者引脚共用；调试图传时应停掉屏幕整图刷新。
